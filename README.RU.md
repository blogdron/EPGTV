 ...
 * `g` -  Принудительно загружает весь имеющийся кэш для поиска телеканала в любом плейлисте (может быть полезно)
...
Кэш может работать в двух режимах, полностью загружаться в память, или
...
В некоторых случаях когда есть файл кеша, но нет файла индекса кэша, то потребуется
