...
`EPGTV` открывает `M3U` файл или `URL` ссылку и автоматически загружает `EPG` данные в свой кеш.
После сохранения кеша сразу отображает программу телепередач на сегодня и завтра, если доступно.
Если доступно то `EPGTV` отображает описание телепередач. Первый старт может быть долгим
так как данные ещё не собраны, последующие запуски будут быстрее за счёт переиспользования кеша.

 * Работает на `mpv 0.38,0.39,0.40,0.41`
 ...
 * `u` -  Обновляет программу телепередач текущего плейлиста (кеш не из плейлиста будет выгружен)
 * `g` -  Принудительно загружает весь имеющийся кеш для поиска телеканала в любом плейлисте (может быть полезно)
 * `esc` - Скрыть отображаемую информацию

`EPGTV` для более быстрой работы использует кеш, он автоматически создаётся
...
нужно **обновить кеш вручную** нажав `u` если у источника `EPG` данных было обновление
...
Кеш может работать в двух режимах, полностью загружаться в память, или
динамически подгружать данные только того телеканала который выбран, по умолчанию
сейчас используется второй вариант, так как кеш может измеряться в гигабайтах,
вместо загрузки всего кеша в память, загружается небольшой индексный файл который
ссылается на срезы данных внутри большого кеш файла и просто считывает когда нужно
нужный кусок данных для разбора и отображения, тем самым не нужно ждать подгрузки
всего кеша, особенно если он велик, это поведение можно поменять в конфиге.

В некоторых случаях когда есть файл кеша, но нет файла индекса кеша, то потребуется
разовая генерация индекса кеша, это произойдёт автоматически и один раз, последующее
открытие и отображение программ телепередач будет ЗНАЧИТЕЛЬНО быстрее.
...
Данные кеша ТВ телепередач находятся тут
